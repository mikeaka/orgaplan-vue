<template>
  <v-container class="grey lighten-5" fluid>
      ...
      <div class="text-center ma-5">
        ...
        <v-flex>
          <p v-if="siteSelection">
            ...
            <strong class="red--text text--lighten-1">{{ siteSelection }}</strong>
          </p>
    ...
    <div v-for="chantier in constructionSites" :key="chantier.id">
      <v-layout class="ma-1" row justify-space-between v-if="siteSelection === chantier.siteName">
        <v-flex class="mt-6">
          <v-card class="text-xs-center pa-2" max-width="350" width="340" flat>
            <v-card-title class="text-xs-center justify-center">Aire de Livraison</v-card-title>
            <v-list-item v-for="deliveryArea in deliveryAreas" :Key="deliveryArea.id">
              <v-btn
                block
                color="primary"
                outlined
          ...
          <v-card class="text-xs-center mt-4" max-width="344" width="340" flat>
            <v-card-title class="text-xs-center justify-center">Materiels Disponible</v-card-title>
            <v-list-item v-for="materiel in siteMaterials" :Key="materiel.id">
              <v-btn block color="primary" outlined :disabled="!materiel.active">{{ materiel.name }}</v-btn>
            </v-list-item>
          </v-card>
        </v-flex>
        <v-flex>
          ...
          <v-card class="mx-auto" flat>
            ...
            >
              ...
              <v-btn color="purple" dark small absolute top right>zone de livraison 2</v-btn>
            </v-img>
          </v-card>
        </v-flex>
      </v-layout>
